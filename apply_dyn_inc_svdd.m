...
function [overall_thresholds, change_points, ratios, W, w] = apply_dyn_inc_svdd( data, columns, block_size, step_size, C, ktype, kpar, high_threshold, low_threshold )
    ...
    if nargin < 8; high_threshold = 1.3; end
    if nargin < 9; low_threshold  = 0.7; end
...
%     thresholds          = results_zeros;
    ...
    change_points = [];
    overall_thresholds = zeros(first_block_size, 1)';
    partial_thresholds = [];
    ratios = ones(first_block_size, 1)';
    
%     clear_persistance = false;
...
%         if counter == 1
%             clear_persistance = true;
%         end

        % Draw mapping and points
%         [h_data, h_SVs, h_new_points, h_outliers, h_boundary, properties] = calculate_data_and_boundary(data, from:i, w0, w, i-step_size+1:i, clear_persistance);
...
%         thresholds(counter,:)         = [time properties('thresholds')];
        ...
        partial_thresholds(end+1) = w.threshold;
        
        
        % Dynamically check for change points
        [is_cp, index, r] = is_changepoint(partial_thresholds, high_threshold, low_threshold, counter, block_size, C);
        
        ratios(end+1) = r;
        is_cp = false;
        if is_cp
           index
           change_points(end+1) = index;
           
           overall_thresholds = [overall_thresholds partial_thresholds];
           partial_thresholds = [];
           
           % Reset the SVDD model
           new_block_size = first_block_size; %ceil(first_block_size * C) + 1;
           end_new_block = min(i+new_block_size, length(data));
           new_block = data(i+1:end_new_block,columns);
           size(new_block)
           if size(new_block, 1) >= first_block_size
           
            W = inc_setup('svdd', ktype, kpar, C, new_block, ones(length(new_block), 1) );
           end
...
function [is_cp, index, r] = is_changepoint(series, high, low, i, block_size, fraction)
    is_cp = false;
    index = 0;
...
%     end
    ...
    if r > high
        is_cp = true;
        index = i + block_size; % + fraction*block_size;
    end
    
    if r < low
        is_cp = true;
        index = i + block_size; %+ fraction*block_size; % - block_size; % + fraction*block_size;
    end
    index;
    is_cp;
